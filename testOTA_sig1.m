...
caseNo =1; %1: constant-amp, 2: two-DOF
...
opt = 1; %1:constant DT ML, 2: constant Rev ML, 3: VKF, 4:: const DT Blough TVDFT, 5: const Rev Blough TVDFT
...
rm=[150;150;180]; %VKF weighting factor, 
...
if opt == 1 || opt == 4 % constant DT
    ...
    while (st+bsize) <= cpos(nr)
        ...
        for i =1:No         
            ...
            speedCorr=0;
            %if(nblocks >= 65 && nblocks <78)
            if(nblocks >= 88 && nblocks < 96)
...
%     FRFOrder = FRFOrder(1:nblocks,:,:);
    blktime = dt*(0:Np-1);
%     figure()
    %plot(freqBandOrder(:,1),abs(FRFOrder(:,1,1)));ylim([0 max(abs(FRFOrder(:,1,1)))])
%     plot(freqBandOrder(:,1),abs(FRFOrder(:,1,1)));ylim([0 max(abs(FRFOrder(:,1,1)))])
%     figure()
%     plot(freqBandOrder(:,2),abs(FRFOrder(:,1,2)));ylim([0 max(abs(FRFOrder(:,1,2)))])
%     figure()
%     plot(blktime,abs(FRFOrder(:,1,3)));ylim([0 max(abs(FRFOrder(:,1,3)))])
%     figure()
%     plot(abs(FRFOrder(:,1,1)));ylim([0 max(abs(FRFOrder(:,1,1)))])
%     figure()
%     plot(abs(FRFOrder(:,1,2)));ylim([0 max(abs(FRFOrder(:,1,2)))])
%     figure()
%     plot(abs(FRFOrder(:,1,3)));ylim([0 max(abs(FRFOrder(:,1,3)))]);
     figure()
    %plot(freqBandOrder(:,1),abs(FRFOrder(:,1,1)));ylim([0 max(abs(FRFOrder(:,1,1)))])
     plot(blktime(Nst:bsize:Nend),abs(FRFOrder(Nst:bsize:Nend,1)));ylim([0 max(abs(FRFOrder(:,1,1)))])
     hold on
     plot(blktime(Nst:Nend),actamp(Nst:Nend,1));ylim([0 1.1*max(abs(actamp(Nst:Nend,1)))])
     figure()
     ...
     plot(blktime(Nst:bsize:Nend),abs(FRFOrder(Nst:bsize:Nend,1)),'-bs','LineWidth',1,'MarkerEdgeColor','b', 'MarkerSize',3,'MarkerIndices',1:6:endplot);ylim([2 1.1*max(abs(FRFOrder(Nst:bsize:Nend,1,1)))]);
     hold on
     plot(blktime(Nst:bsize:Nend),abs(FRFOrder(Nst:bsize:Nend,2)),'-rd','LineWidth',1,'MarkerEdgeColor','r', 'MarkerSize',3,'MarkerIndices',1:6:endplot)
     plot(blktime(Nst:bsize:Nend),abs(FRFOrder(Nst:bsize:Nend,3)),'-go','LineWidth',1,'MarkerEdgeColor','g', 'MarkerSize',3,'MarkerIndices',1:6:endplot)
...
%      ylabel({'Amp.'}, 'FontSize', 10);
    set(gcf,'position',[400,400,320,240])
    ylabel('Amp. (mm)');xlabel('time (s)')
    legend('o1=1X','o2=1.2X','o3=40Hz');
...
if opt == 2 || opt == 5 % constant number of rev     
    ...
    %FRF=zeros(nblocks,No,1); 
    FRFOrder = zeros(nblocks,No,NTraces);
    freqBandOrder = zeros(nblocks,NTraces);
    blktime=zeros(nblocks,1);
    nblocks = 0;
    st =  2;%starting cycle 
    ...
    while (st+bnr) <= nr
        ...
        for i =1:No
            ...
            for j=1:NTraces
                if TraceType(j) >= 0 %include only orders, discard const freq orders
                   FRFOrder(nblocks,i,k) = OA(j) + 1i*OB(j);% Cn =0.5(an - i bn)  
                   k = k+1;
                end
            end
...
%     plot(blktime,abs(FRFOrder(:,1,3)));ylim([0 max(abs(FRFOrder(:,1,3)))]);
    ...
    plot(blktime,abs(FRFOrder(:,1,1)));ylim([0 max(abs(FRFOrder(:,1,1)))])
    hold on
     plot(dt*(Nst:Nend),env1x(Nst:Nend));ylim([0 1.1*max(abs(env1x(Nst:Nend)))])
    figure()
    plot(blktime,abs(FRFOrder(:,1,2)));ylim([0 max(abs(FRFOrder(:,1,2)))])
    figure()
    plot(blktime,abs(FRFOrder(:,1,3)));ylim([0 max(abs(FRFOrder(:,1,3)))]);
...
%      Np=2000;
    ...
    if NTraces == 2
        ...
        endplot=fix(Nend/bsize);
        plot(blktime(Nst:bsize:Nend),abs(FRFOrder(Nst:bsize:Nend,1)),'-bs','LineWidth',1,'MarkerEdgeColor','b', 'MarkerSize',3,'MarkerIndices',1:6:endplot);ylim([2 max(1.1*abs(FRFOrder(Nst:bsize:Nend,1,1)))]);
        hold on
        plot(blktime(Nst:bsize:Nend),abs(FRFOrder(Nst:bsize:Nend,2)),'-rd','LineWidth',1,'MarkerEdgeColor','r', 'MarkerSize',3,'MarkerIndices',1:6:endplot)
        plot(blktime(Nst:bsize:Nend),abs(FRFOrder(Nst:bsize:Nend,3)),'-go','LineWidth',1,'MarkerEdgeColor','g', 'MarkerSize',3,'MarkerIndices',1:6:endplot)
%      plot(blktime,abs(FRFOrder(:,1,1)));ylim([0 max(abs(FRFOrder(:,1,1)))]);
