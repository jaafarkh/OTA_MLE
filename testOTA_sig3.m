...
bsize = 75;%processing length for constant DT in samples, 4800
...
%% begin OTA
No = 1;
bsizeMat=[50 75 100 125 150 175 225 275 330 360];
...
%      bsize=2*bsize;
     ...
     hold on
     plot(blktime(Nst:bsize:Nend),abs(FRFOrder(Nst:bsize:Nend,2)),'-rd','LineWidth',0.5,'MarkerEdgeColor','r', 'MarkerSize',3,'MarkerIndices',1:6:endplot);ylim([0 1.1*max(abs(FRFOrder(Nst:Nend,2,:)))]);
     if(NTraces > 2)
...
%        MPEA=100*max(abs(actamp(Nst:Nend,:)-abs(FRFOrder(Nst:Nend,:))))./max(actamp(Nst:Nend,:))
      ...
      bsize=50;
      endplot=fix(Nend/bsize);
      plot(blktime(Nst:bsize:Nend),abs(FRFOrder(Nst:bsize:Nend,1)),'-bs','LineWidth',0.5,'MarkerEdgeColor','b', 'MarkerSize',3,'MarkerIndices',1:18:endplot);ylim([0 max(1.1*abs(FRFOrder(Nst:Nend,1)))]);
      hold on
      plot(blktime(Nst:bsize:Nend),abs(FRFOrder(Nst:bsize:Nend,2)),'-rd','LineWidth',0.5,'MarkerEdgeColor','r', 'MarkerSize',3,'MarkerIndices',1:18:endplot);ylim([0 max(1.1*abs(FRFOrder(Nst:Nend,2)))]);
      if NTraces>2
        plot(blktime(Nst:bsize:Nend),abs(FRFOrder(Nst:bsize:Nend,3)),'-go','LineWidth',0.5,'MarkerEdgeColor','g', 'MarkerSize',3,'MarkerIndices',1:18:endplot)
      end
      set(gcf,'position',[200,200,400,300])
      ylabel('Amp. (mm)');xlabel('time (s)')
      legend('o1=1X','o2=4X','o3=46Hz');
