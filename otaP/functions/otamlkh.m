function [OA,OB] = otamlkh(x,N,TraceType,TraceOrder,NTraces,Theta,StP,deltaT,showerr)
...
if(showerr > 0)
    ...
    for tr=2:NT
        ...
        for j=1:NTraces
            if TraceType(j) == 0 %order
                Phi(:,j) = TraceOrder(j)*(Theta(:)+(cg/LT)*deltaT*(0:N-1)');%*deltaT*(0:N-1)'
            end
            if TraceType(j) == 1 %fixed frequency in Hz
                Phi(:,j) = 2 * pi * TraceOrder(j) * (StP + (0:N-1)) * deltaT;
            end
...
%         fprintf('TR, err, cg, g  :  %2d, %16.8f %10.5f %10.5f  \n',tr,cerr,cg,g);    
    ...
    for j=1:NTraces
        if TraceType(j) == 0 %order
            Phi(:,j) = TraceOrder(j)*(Theta(:)+(cg/LT)*deltaT*(0:N-1)');
        end
        if TraceType(j) == 1 %fixed frequency in Hz
            Phi(:,j) = 2 * pi * TraceOrder(j) * (StP + (0:N-1)) * deltaT;
        end
