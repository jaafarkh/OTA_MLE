function [x,bw] = vkmmy(y,f,fs,r,filtord)
% Modified by Jaafar by using direct solution instead of pcg
...
elseif filtord == 1
 ...
 bw = fs*(0.0652097315./r).^0.5;
 T = 2.85*r.^0.5;
elseif filtord == 2
 NR = N-3;
 e = ones(NR,1);
 A = spdiags([e -3*e 3*e -e],0:3,NR,N);
 bw = fs*(0.020756902./r).^(1/3);
 T =  2.80*r.^(1/3);
else
